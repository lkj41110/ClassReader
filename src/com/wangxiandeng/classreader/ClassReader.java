...
public class ClassReader {
    ...
    public void read(String classPath) {
        ...
        try {
            ...
            parseClassInfo(inputStream);
            parseSuperClassInfo(inputStream);
            parseInterfaceInfo(inputStream);
            parseMemberInfo(inputStream);
            parseMethodInfo(inputStream);
    ...
    //解析常量池
    private void parseConstantPool(InputStream inputStream) {
    ...
    //获取类信息
    private void parseClassInfo(InputStream inputStream){
        classFile.accessFlag = U2.read(inputStream);
        int classIndex = U2.read(inputStream);
        ConstantClass clazz = (ConstantClass) classFile.constantPool.cpInfo[classIndex];
        ConstantUtf8 className = (ConstantUtf8) classFile.constantPool.cpInfo[clazz.nameIndex];
        classFile.className = className.value;
    }

    //获取父类信息
    private void parseSuperClassInfo(InputStream inputStream){
        int superIndex = U2.read(inputStream);
        ConstantClass superClazz = (ConstantClass) classFile.constantPool.cpInfo[superIndex];
        ConstantUtf8 superclassName = (ConstantUtf8) classFile.constantPool.cpInfo[superClazz.nameIndex];
        classFile.superClass = superclassName.value;
    }

    //获取接口信息
    private void parseInterfaceInfo(InputStream inputStream){
        ...
        for (int i = 0; i < classFile.interfaceCount; i++) {
            ...
            ConstantClass interfaceClazz = (ConstantClass) classFile.constantPool.cpInfo[interfaceIndex];
            ConstantUtf8 interfaceName = (ConstantUtf8) classFile.constantPool.cpInfo[interfaceClazz.nameIndex];
    ...
    //获取字段信息
    private void parseMemberInfo(InputStream inputStream){
        ...
        for (int i = 0; i < classFile.fieldCount; i++) {
            MemberInfo fieldInfo = new MemberInfo(classFile.constantPool);
            fieldInfo.read(inputStream);
            System.out.print("field:" + ((ConstantUtf8) classFile.constantPool.cpInfo[fieldInfo.nameIndex]).value + ", ");
            System.out.print("desc:" + ((ConstantUtf8) classFile.constantPool.cpInfo[fieldInfo.descriptorIndex]).value + "\n");
        }
        System.out.print("\n");
    }

    //获取方法信息
    private void parseMethodInfo(InputStream inputStream){
        ...
        for (int i = 0; i < classFile.methodCount; i++) {
            MemberInfo methodInfo = new MemberInfo(classFile.constantPool);
            methodInfo.read(inputStream);
            System.out.print("method:" + ((ConstantUtf8) classFile.constantPool.cpInfo[methodInfo.nameIndex]).value + "(), ");
            System.out.print("desc:" + ((ConstantUtf8) classFile.constantPool.cpInfo[methodInfo.descriptorIndex]).value + "\n");
